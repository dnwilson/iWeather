...
RSpec.describe Forecast, type: :model do
  describe ".build_extended_forecast" do
    ...
    it do
      ...
      current = build_extended_forecast.first.current_period
    ...
    it "returns the correct period" do
      period = build_extended_forecast.first.current_period
      expect(period.start_time).to eq(current_period[:start_time].to_datetime)
      expect(period.end_time).to eq(current_period[:end_time].to_datetime)
      expect(period.temperature).to eq(current_period[:temperature])
      expect(period.short_forecast).to eq(current_period[:short_forecast])
      expect(period.icon).to eq(current_period[:icon])
    end
  end
end
